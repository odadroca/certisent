...
<div class="card max-w-xl">
  <div class="card-body">
    <h1 class="page-title mb-4"><?php echo t('monitor_edit.heading'); ?></h1>
    <?php if ($err): ?>
    ...
    <form method="post" class="space-y-3">
      ...
      <div>
        ...
        <input name="url" class="form-input" value="<?php echo h($_POST['url'] ?? $m['url']); ?>" />
      </div>
      <div class="grid-2 gap-3">
        <div>
          <label class="form-label"><?php echo t('monitor.common.notify_days'); ?></label>
          <input type="number" name="notify_days" class="form-input" value="<?php echo h((string)($_POST['notify_days'] ?? $m['notify_days_before_expiry'])); ?>" />
        </div>
        <div>
          <label class="form-label"><?php echo t('monitor_edit.check_frequency'); ?></label>
          <input type="number" name="freq_min" class="form-input" value="<?php echo h((string)($_POST['freq_min'] ?? $m['check_frequency_minutes'])); ?>" />
        </div>
      </div>

      <div>
        <label class="form-label"><?php echo t('monitor_edit.tls_validation_mode'); ?></label>
        <?php $curTvm = (string)($_POST['tls_validation_mode'] ?? ($m['tls_validation_mode'] ?? 'off')); ?>
        <select name="tls_validation_mode" class="form-select">
        ...
        <div class="form-help"><?php echo t('monitor_edit.tls_validation_note'); ?></div>
      </div>

      <div class="mt-1">
        <label class="form-label"><?php echo t('monitor_edit.pin_mode'); ?></label>
        <?php $curPm = (string)($_POST['pin_mode'] ?? ($m['pin_mode'] ?? 'off')); ?>
        <select name="pin_mode" class="form-select">
        ...
        <div class="form-help"><?php echo t('monitor_edit.pin_note'); ?></div>
      </div>

      <div>
        <label class="form-label"><?php echo t('monitor_edit.pin_value'); ?></label>
        <input name="pin_spki_sha256" class="form-input font-mono text-xs" placeholder="sha256/&lt;base64&gt;" value="<?php echo h((string)($_POST['pin_spki_sha256'] ?? ($m['pin_spki_sha256'] ?? ''))); ?>" />
        <div class="form-help"><?php echo t('monitor_edit.pin_value_note'); ?></div>
      </div>
      <label class="form-checkbox">
        <input type="checkbox" name="enabled" <?php echo ((int)($m['enabled'])===1 ? 'checked' : ''); ?> />
        <span><?php echo t('common.enabled'); ?></span>
      </label>

      <div class="grid-2 gap-3">
        <label class="form-checkbox">
          <input type="checkbox" name="notify_on_change" <?php echo ((int)($m['notify_on_change'])===1 ? 'checked' : ''); ?> />
          <span><?php echo t('monitor_edit.notify_on_change'); ?></span>
        </label>
        <label class="form-checkbox">
          <input type="checkbox" name="notify_on_renewal" <?php echo ((int)($m['notify_on_renewal'])===1 ? 'checked' : ''); ?> />
          <span><?php echo t('monitor_edit.notify_on_renewal'); ?></span>
        </label>
      </div>
      <button class="btn btn-primary"><?php echo t('common.save'); ?></button>
