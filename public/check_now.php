...
<div class="card">
  <div class="card-body">
    <h1 class="page-title mb-3">Quick check result</h1>
      ...
      <?php if (is_array($hv) && (($hv['ok'] ?? true) === false) && (($hv['error'] ?? '') === 'hostname_mismatch')): ?>
        <div class="alert alert-warn">
          ...
          <?php if (!empty($hv['candidates']) && is_array($hv['candidates'])): ?>
            ...
            <?php if (count($c) > 0): ?>
              <div class="mt-1 text-xs">Names on cert: <span class="font-mono text-break"><?php echo h(implode(', ', $c)); ?></span></div>
            <?php endif; ?>
          <?php endif; ?>
        </div>
      <?php endif; ?>
      ...
      <?php if (is_array($tv) && (($tv['ok'] ?? true) === false) && (($tv['type'] ?? '') === 'untrusted')): ?>
        ...
        <div class="alert alert-warn">
          ...
          <?php if (!empty($tv['error'])): ?>
            <div class="mt-1 text-xs">Details: <span class="font-mono text-break"><?php echo h((string)$tv['error']); ?></span></div>
          <?php endif; ?>
        </div>
      <?php endif; ?>
      <div class="grid-2 text-sm">
        <div><span class="detail-label">URL</span><div class="font-mono"><?php echo h($res['url']); ?></div></div>
        <div><span class="detail-label">Fingerprint (SHA-256)</span><div class="font-mono text-break"><?php echo h($res['fingerprint']); ?></div></div>
        <div><span class="detail-label">SPKI sha256 (pin)</span><div class="font-mono text-break"><?php echo h($res['spki_sha256_base64'] ? ('sha256/'.$res['spki_sha256_base64']) : ''); ?></div></div>
        <div><span class="detail-label">Issuer</span><div><?php echo h((string)$res['issuer']); ?></div></div>
        <div><span class="detail-label">Subject</span><div><?php echo h((string)$res['subject']); ?></div></div>
        <div><span class="detail-label">Valid from</span><div><?php echo h((string)$res['valid_from']); ?> UTC</div></div>
        <div><span class="detail-label">Valid to</span><div><?php echo h((string)$res['valid_to']); ?> UTC</div></div>
        <div><span class="detail-label">Days remaining</span><div class="font-semibold"><?php echo h((string)$res['days_remaining']); ?></div></div>
      </div>
    <?php endif; ?>

    <div class="mt-6">
      <a href="<?php echo current_user() ? 'dashboard.php' : 'index.php'; ?>">Back</a>
    </div>
  </div>
</div>
<?php render_footer(); ?>
